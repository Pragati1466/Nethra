...
import { S as weeklyPerformance, a as calibrationBins, d as historicalLedger, g as predVsActualSeries, m as learningSummary, p as learnRecords } from "./intel-Bd2vThK7.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/learn-CsAk_KPM.js
...
function LearnPage() {
	...
	const [chartDataReady, setChartDataReady] = (0, import_react.useState)(false);
	const chartDataReadyTimeoutRef = (0, import_react.useRef)(null);
	...
	const [scatter, setScatter] = (0, import_react.useState)([]);
	const [weekly, setWeekly] = (0, import_react.useState)([]);
	const [calib, setCalib] = (0, import_react.useState)([]);
	...
	const filteredLedger = (0, import_react.useMemo)(() => {
		...
		return safeLedger.filter((r) => r.id.toLowerCase().includes(query) || r.event.toLowerCase().includes(query) || r.status.toLowerCase().includes(query));
	}, [ledger, debouncedSearchQuery]);
	const filteredRecords = (0, import_react.useMemo)(() => {
		if (!debouncedSearchQuery) return safeRecords;
		const query = debouncedSearchQuery.toLowerCase();
		return safeRecords.filter((r) => r.id.toLowerCase().includes(query) || r.name.toLowerCase().includes(query) || r.notes.toLowerCase().includes(query));
