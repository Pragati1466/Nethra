...
import { subscribeClosedIntel, getLatestClosedIntel, type ClosedEventIntel } from "@/lib/closed_intel_store";
...
function LearnPage() {
  ...
  const [cei, setCei] = useState<ClosedEventIntel | null>(getLatestClosedIntel());
  // Defer heavy chart computations so the route becomes interactive faster.
  // No UI change: we render the same charts; we just compute their data after mount.
  const [chartDataReady, setChartDataReady] = useState(false);
  const chartDataReadyTimeoutRef = useRef<number | null>(null);
  useEffect(() => {
    // Defer heavy chart datasets and allow the route to paint.
    // Small delay prevents blocking the main thread during initial mount.
    chartDataReadyTimeoutRef.current = window.setTimeout(() => setChartDataReady(true), 50);
    return () => {
      if (chartDataReadyTimeoutRef.current) window.clearTimeout(chartDataReadyTimeoutRef.current);
    };
  }, []);
  useEffect(() => {
  ...
  // Cache derived series so we don't recompute heavy CPU work more than once per navigation.
  // NOTE: Defer computing expensive chart datasets until after mount.
  // IMPORTANT: compute the two charts you care about first (scatter + calibration), then the rest.
  // This does not change UI structure or outputs; it only reduces the time-to-first-render for the graphs.
  // Typed chart datasets (arrays), not function references.
  const [scatter, setScatter] = useState<ReturnType<typeof predVsActualSeries>>([]);
  const [weekly, setWeekly] = useState<ReturnType<typeof weeklyPerformance>>([]);
  const [calib, setCalib] = useState<ReturnType<typeof calibrationBins>>([]);
  const [ledger, setLedger] = useState<ReturnType<typeof historicalLedger>>([]);
  const [records, setRecords] = useState<ReturnType<typeof learnRecords>>([]);


  useEffect(() => {
    if (!chartDataReady) return;

    // 1) Scatter + Calibration first
    setScatter(predVsActualSeries());
    setWeekly([]); // keep empty until step 2
    setCalib(calibrationBins());
    setLedger([]);
    setRecords([]);


    // 2) Remaining datasets after a short yield
    const id = window.setTimeout(() => {
      setWeekly(weeklyPerformance());
      setLedger(historicalLedger(14));
      setRecords(learnRecords());
    }, 50);

    return () => window.clearTimeout(id);
  }, [chartDataReady]);

  const safeScatter = scatter;
  const safeWeekly = weekly;
  const safeCalib = calib;
  ...
  const filteredLedger = useMemo(() => {
    if (!debouncedSearchQuery) return safeLedger;
    const query = debouncedSearchQuery.toLowerCase();
    return safeLedger.filter(
      (r) =>
  ...
  const filteredRecords = useMemo(() => {
    if (!debouncedSearchQuery) return safeRecords;
    const query = debouncedSearchQuery.toLowerCase();
    ...
    <AppShell>
      <div className="p-4 lg:p-6 grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 grid grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <MetricStat
            label="Re-tunings"
            value={Math.max(1, records.filter((r) => Math.abs(r.predictedRisk - r.actualRisk) > 8).length)}
            tone="warning"
            sub="priors updated"
          />
        </div>
          ...
          <div className="h-[320px] p-3">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 10, right: 20, bottom: 30, left: 10 }}>
                ...
                <Scatter name="Events" data={safeScatter} fill={C_PRED} fillOpacity={0.7} />
              </ScatterChart>
            </ResponsiveContainer>
          </div>
        </Panel>
        ...
        >
          <div className="h-[320px] p-3">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={safeCalib} margin={{ top: 10, right: 20, bottom: 30, left: 0 }}>
                <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" />
        ...
        >
          <div className="h-[300px] p-3">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={safeWeekly} margin={{ top: 10, right: 20, bottom: 20, left: 0 }}>
                <defs>
        ...
        {/* Latest Learned Event card — bound to shared Closed Event Intelligence record */}
        <Panel
        ...
        >
          <div className="p-4 space-y-2 text-sm">
            ...
            <div className="rounded-md border border-border bg-card/40 p-3 space-y-2">
              <div>
                <span className="font-medium">Event:</span> {cei?.eventId ? `${cei.eventId} · ${cei.eventId === "EVT-2041" ? "RCB vs CSK — IPL Match" : "Event impact in queue"}` : "EVT-2041 · RCB vs CSK — IPL Match"}
              </div>
              ...
              <div className="pt-1">
                ...
                <div className="text-[12px] text-muted-foreground mt-1">EIS: {98}</div>
              ...
              <div className="pt-1">
                <div className="font-medium">T−IW Drift:</div>
                <div className="text-[12px] text-muted-foreground mt-1">{cei?.timeWindowRemainingMin ? "Bellary Rd · Hebbal" : "Bellary Rd · Hebbal"}</div>
                <div className="text-[12px] text-muted-foreground">Forecast deviation detected</div>
                <div className="text-[12px] text-muted-foreground">T−IW: {cei?.timeWindowRemainingMin ?? 16} min</div>
              ...
              <div className="pt-1">
                <div className="font-medium">Revised Action:</div>
                <div className="text-[12px] text-muted-foreground mt-1">{cei?.revisedAction?.split(" · ")[0] ?? "Diversion activated"}</div>
                <div className="text-[12px] text-muted-foreground">12 additional officers deployed</div>
              </div>

              <div className="pt-1">
                <div className="font-medium">Actual Outcome:</div>
                <div className="text-[12px] text-muted-foreground mt-1">Actual Delay: {cei?.actualDelayMin ?? "—"} min</div>
                <div className="text-[12px] text-muted-foreground">
                  Improvement: {cei?.forecastDelayMin && cei?.actualDelayMin ? Math.max(0, cei.forecastDelayMin - cei.actualDelayMin) : "—"} min
                </div>
              </div>

              <div className="pt-1">
                <div className="font-medium">Learned:</div>
                <div className="text-[12px] text-muted-foreground mt-1">Match-day prior updated</div>
                <div className="text-[12px] text-muted-foreground">Junction DNA recalibrated</div>
                <div className="text-[12px] text-muted-foreground">Future forecasts strengthened</div>
              </div>
            </div>
          </div>
        </Panel>
          ...
          <div className="overflow-x-auto">
            {filteredLedger.length === 0 ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredLedger.map((r) => ( /* filteredLedger depends on ledger */
